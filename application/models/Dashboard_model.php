...
class Dashboard_model extends Ci_Model{
  ...
  function get_maxd($pl){
    ...
    $select = array(
        'a.nama_gedung',
        'count(b.id_karyawan) as total_karyawan'
    );

    $this->db->select($select);
    $this->db->from('karyawan as b');
    $this->db->join('gedung as a','b.gedung_id=a.gedung_id');
    $this->db->group_by('b.gedung_id,a.nama_gedung');
    $this->db->order_by('total_karyawan','DESC');
   ...
   function get_max2($in){
    ...
    $this->db->select($select);
    $this->db->from('karyawan as b');
    $this->db->join('jabatan as a','b.jabatan=a.id_jabatan');
    $this->db->group_by('b.jabatan,a.nama_jabatan');
    $this->db->order_by('total_karyawan','DESC');
